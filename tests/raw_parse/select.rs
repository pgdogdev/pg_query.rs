...
#[test]
fn it_parses_tablesample_bernoulli() {
    let query = "SELECT * FROM users TABLESAMPLE BERNOULLI(10)";
    parse_test!(query);
}

/// Test TABLESAMPLE SYSTEM
#[test]
fn it_parses_tablesample_system() {
    let query = "SELECT * FROM users TABLESAMPLE SYSTEM(25)";
    parse_test!(query);
}

/// Test TABLESAMPLE with REPEATABLE
#[test]
fn it_parses_tablesample_repeatable() {
    let query = "SELECT * FROM users TABLESAMPLE SYSTEM(10) REPEATABLE(42)";
    parse_test!(query);
}

// ============================================================================
// XMLTABLE / RangeTableFunc tests
// ============================================================================

/// Test XMLTABLE basic
#[test]
fn it_parses_xmltable() {
    let query = "SELECT * FROM XMLTABLE('/root/row' PASSING my_xml COLUMNS id int PATH '@id', name text PATH 'name')";
    parse_test!(query);
}

/// Test XMLTABLE with namespaces
#[test]
fn it_parses_xmltable_namespaces() {
    let query = "SELECT * FROM XMLTABLE(XMLNAMESPACES ('http://example.com' AS x), '/x:root/x:row' PASSING my_xml COLUMNS id int PATH '@id')";
    parse_test!(query);
}

// ============================================================================
// Range functions in FROM clause
// ============================================================================

/// Test generate_series in FROM
#[test]
fn it_parses_generate_series_from() {
    let query = "SELECT * FROM generate_series(1, 10)";
    parse_test!(query);
}

/// Test WITH ORDINALITY
#[test]
fn it_parses_with_ordinality() {
    let query = "SELECT * FROM generate_series(1, 5) WITH ORDINALITY";
    parse_test!(query);
}

/// Test ROWS FROM
#[test]
fn it_parses_rows_from() {
    let query = "SELECT * FROM ROWS FROM (generate_series(1, 3), generate_series(4, 6))";
    parse_test!(query);
}

/// Test function returning table with column definitions
#[test]
fn it_parses_function_with_col_defs() {
    let query = "SELECT * FROM get_rows() AS t(id int, name text)";
    parse_test!(query);
}
...
#[test]
fn it_parses_create_table_like() {
    let query = "CREATE TABLE users_copy (LIKE users)";
    parse_test!(query);
}

/// Test CREATE TABLE LIKE INCLUDING ALL
#[test]
fn it_parses_create_table_like_including() {
    let query = "CREATE TABLE users_copy (LIKE users INCLUDING ALL)";
    parse_test!(query);
}

/// Test CREATE TABLE LIKE INCLUDING specific items
#[test]
fn it_parses_create_table_like_specific() {
    let query = "CREATE TABLE users_copy (LIKE users INCLUDING CONSTRAINTS EXCLUDING INDEXES)";
    parse_test!(query);
}

/// Test INHERITS
#[test]
fn it_parses_create_table_inherits() {
    let query = "CREATE TABLE capitals (state char(2)) INHERITS (cities)";
    parse_test!(query);
}
...
#[test]
fn it_parses_create_temp_table() {
    let query = "CREATE TEMP TABLE temp_users (id int, name text)";
    parse_test!(query);
}

/// Test CREATE UNLOGGED TABLE
#[test]
fn it_parses_create_unlogged_table() {
    let query = "CREATE UNLOGGED TABLE unlogged_users (id int, name text)";
    parse_test!(query);
}

/// Test CREATE TEMP TABLE ON COMMIT DROP
#[test]
fn it_parses_create_temp_on_commit() {
    let query = "CREATE TEMP TABLE temp_users (id int) ON COMMIT DROP";
    parse_test!(query);
}

// ============================================================================
// Column identity and generation
// ============================================================================

/// Test GENERATED ALWAYS AS IDENTITY
#[test]
fn it_parses_generated_identity() {
    let query = "CREATE TABLE users (id int GENERATED ALWAYS AS IDENTITY, name text)";
    parse_test!(query);
}

/// Test GENERATED BY DEFAULT AS IDENTITY
#[test]
fn it_parses_generated_by_default_identity() {
    let query = "CREATE TABLE users (id int GENERATED BY DEFAULT AS IDENTITY (START WITH 1000), name text)";
    parse_test!(query);
}

/// Test GENERATED computed column
#[test]
fn it_parses_generated_computed() {
    let query = "CREATE TABLE people (first_name text, last_name text, full_name text GENERATED ALWAYS AS (first_name || ' ' || last_name) STORED)";
    parse_test!(query);
}
...
#[test]
fn it_parses_deferrable_fk() {
    let query = "CREATE TABLE orders (id int, user_id int REFERENCES users(id) DEFERRABLE INITIALLY DEFERRED)";
    parse_test!(query);
}

/// Test EXCLUDE constraint
#[test]
fn it_parses_exclude_constraint() {
    let query = "CREATE TABLE reservations (id int, during tsrange, EXCLUDE USING gist (during WITH &&))";
    parse_test!(query);
}

/// Test column CHECK constraint
#[test]
fn it_parses_column_check() {
    let query = "CREATE TABLE products (price numeric CONSTRAINT positive_price CHECK (price > 0))";
    parse_test!(query);
}

/// Test multi-column UNIQUE
#[test]
fn it_parses_multi_column_unique() {
    let query = "CREATE TABLE orders (id int, user_id int, code text, UNIQUE (user_id, code))";
    parse_test!(query);
}

/// Test FK ON UPDATE / ON DELETE
#[test]
fn it_parses_fk_actions() {
    let query = "CREATE TABLE orders (id int, user_id int REFERENCES users(id) ON UPDATE CASCADE ON DELETE SET NULL)";
    parse_test!(query);
}

// ============================================================================
// INSERT INTO SELECT with RETURNING and complex CTE
// ============================================================================

/// Test complex SELECT with INTO
#[test]
fn it_parses_select_into() {
    let query = "SELECT id, name INTO new_users FROM users WHERE created_at > '2023-01-01'";
    parse_test!(query);
}

/// Test SELECT INTO TEMP
#[test]
fn it_parses_select_into_temp() {
    let query = "SELECT * INTO TEMP tmp_active FROM users WHERE active = true";
    parse_test!(query);
}

// ============================================================================
// ALTER TABLE operations (more variants)
// ============================================================================

/// Test ALTER TABLE ALTER COLUMN TYPE
#[test]
fn it_parses_alter_column_type() {
    let query = "ALTER TABLE users ALTER COLUMN id TYPE bigint";
    parse_test!(query);
}

/// Test ALTER TABLE ALTER COLUMN TYPE USING
#[test]
fn it_parses_alter_column_type_using() {
    let query = "ALTER TABLE users ALTER COLUMN age TYPE int USING age::int";
    parse_test!(query);
}

/// Test ALTER TABLE SET DEFAULT
#[test]
fn it_parses_alter_column_set_default() {
    let query = "ALTER TABLE users ALTER COLUMN status SET DEFAULT 'active'";
    parse_test!(query);
}

/// Test ALTER TABLE DROP DEFAULT
#[test]
fn it_parses_alter_column_drop_default() {
    let query = "ALTER TABLE users ALTER COLUMN status DROP DEFAULT";
    parse_test!(query);
}

/// Test ALTER TABLE ALTER COLUMN SET NOT NULL
#[test]
fn it_parses_alter_column_set_not_null() {
    let query = "ALTER TABLE users ALTER COLUMN email SET NOT NULL";
    parse_test!(query);
}

/// Test ALTER TABLE DROP CONSTRAINT
#[test]
fn it_parses_alter_drop_constraint() {
    let query = "ALTER TABLE users DROP CONSTRAINT IF EXISTS unique_email";
    parse_test!(query);
}

/// Test ALTER TABLE ENABLE/DISABLE TRIGGER
#[test]
fn it_parses_alter_enable_trigger() {
    let query = "ALTER TABLE users ENABLE TRIGGER audit_trigger";
    parse_test!(query);
}

/// Test ALTER TABLE DISABLE ROW LEVEL SECURITY
#[test]
fn it_parses_alter_disable_rls() {
    let query = "ALTER TABLE users DISABLE ROW LEVEL SECURITY";
    parse_test!(query);
}

/// Test ALTER TABLE ATTACH PARTITION
#[test]
fn it_parses_alter_attach_partition() {
    let query = "ALTER TABLE measurements ATTACH PARTITION measurements_2024 FOR VALUES FROM ('2024-01-01') TO ('2025-01-01')";
    parse_test!(query);
}

/// Test ALTER TABLE DETACH PARTITION
#[test]
fn it_parses_alter_detach_partition() {
    let query = "ALTER TABLE measurements DETACH PARTITION measurements_2020";
    parse_test!(query);
}

/// Test ALTER TABLE INHERIT
#[test]
fn it_parses_alter_inherit() {
    let query = "ALTER TABLE capitals INHERIT cities";
    parse_test!(query);
}
