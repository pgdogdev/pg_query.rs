//! Basic parsing tests for parse_raw_iter.
//!
...
/// Test that parse_raw_iter results can be deparsed back to SQL
#[test]
fn it_deparses_parse_raw_iter_result() {
    let query = "SELECT * FROM users";
    let result = parse_raw_iter(query).unwrap();

    let deparsed = result.deparse().unwrap();
    assert_eq!(deparsed, query);
}

/// Test that parse_raw_iter successfully parses a simple SELECT query
#[test]
...
/// Test that parse_raw_iter handles syntax errors
#[test]
fn it_handles_parse_errors() {
    let query = "SELECT * FORM users";
    let raw_error = parse_raw_iter(query).err().unwrap();
...
/// Test that parse_raw_iter and parse produce equivalent results for simple SELECT
#[test]
...
/// Test that parse_raw_iter and parse produce equivalent results for SELECT with table
#[test]
...
/// Test that parse_raw_iter handles empty queries (comments only)
#[test]
...
/// Test that parse_raw_iter parses multiple statements
#[test]
...
fn it_deparse_raw_simple_select() {
    let query = "SELECT 1";
    let result = parse_raw_iter(query).unwrap();
    let deparsed = deparse_raw(&result.protobuf).unwrap();
    assert_eq!(deparsed, query);
}
...
fn it_deparse_raw_method_on_parse_result() {
    ...
    let result = parse_raw_iter(query).unwrap();
    // Test the method on ParseResult
...
fn it_deparse_raw_method_on_protobuf_parse_result() {
    ...
    let result = parse_raw_iter(query).unwrap();
    // Test the method on protobuf::ParseResult
