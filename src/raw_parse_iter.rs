...
unsafe fn convert_node_boxed(node_ptr: *const bindings_raw::Node) -> Option<Box<protobuf::Node>> {
...
impl Processor {
    ...
    fn get_result(mut self) -> Option<Box<protobuf::Node>> {
        ...
        assert!(self.result_stack.is_empty(), "Result stack should be empty after processing");
        ...
        while let Some(entry) = self.stack.pop() {
            ...
            match node_tag {
                ...
                bindings_raw::NodeTag_T_SelectStmt => {
                    ...
                    if collect {
                        let stmt = self.collect_select_stmt(&*stmt);
                        self.push_result(stmt);
                    } else {
    ...
    fn single_result(&mut self, node: *const bindings_raw::Node) -> Option<protobuf::Node> {
        ...
        Some(self.result_stack.pop().expect("result stack should not be empty while processing"))
    }
    ...
    unsafe fn queue_into_clause(&mut self, ic: *const bindings_raw::IntoClause) {
        ...
        //     col_names: self.convert_list_to_nodes(ic_ref.colNames),
        //     access_method: convert_c_string(ic_ref.accessMethod),
        //     options: self.convert_list_to_nodes(ic_ref.options),
    ...
    unsafe fn fetch_into_clause(&mut self, ic: *const bindings_raw::IntoClause) -> Option<Box<protobuf::IntoClause>> {
        ...
        Some(Box::new(protobuf::IntoClause {
            rel: if ic_ref.rel.is_null() { None } else { Some(convert_range_var(&*ic_ref.rel)) },
            col_names: self.fetch_list_results(ic_ref.colNames),
            access_method: convert_c_string(ic_ref.accessMethod),
            options: self.fetch_list_results(ic_ref.options),
            on_commit: ic_ref.onCommit as i32 + 1,
            table_space_name: convert_c_string(ic_ref.tableSpaceName),
            view_query: self.single_result(ic_ref.viewQuery).map(Box::new),
            skip_data: ic_ref.skipData,
        }))
    }
...
unsafe fn convert_node(node_ptr: *const bindings_raw::Node) -> Option<protobuf::Node> {
    if node_ptr.is_null() {
        return None;
    }
